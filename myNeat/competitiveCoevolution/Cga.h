...
private:
    ...
    // vector of vectors of genomes. These
    // are used to have "copies" of genomes
    // to help support multi-threaded evaluations
    // as we can't have multiple threads using the
    // same copy of a neural network at once. The
    // CGenome class has copy constructors, so duplicate
    // the Genome, and create a new neural network
    // from the genome copy. See Cga::*Duplicate public functions
    vector< vector<CGenome> > popGenomesDupVec;
    vector< vector<CGenome> > bestGenomesDupVec;
    vector< vector<CGenome> > hallOfFameGenomesDupVec;
    vector< vector<CGenome> > speciesLeadersGenomesDupVec;
    vector< vector<CGenome> > customArchiveGenomesDupVec;
...
public:
    ...
    // If you're using multithreaded evaluations, you want to avoid using
    // the same neural network by two processes at once. Let the Cga type
    // create copies for you. Call for each of your threads, so
    // each thread has it's own copy to work with
    vector<CNeuralNet*> GetBestPhenotypesFromLastGenerationDuplicate();
    vector<CNeuralNet*> GetHallOfFamePhenotypesDuplicate();
    vector<CNeuralNet*> GetCustomArchivePhenotypesDuplicate();
    vector<CNeuralNet*> CreatePhenotypesDuplicate();
    vector<CNeuralNet*> GetLeaderPhenotypesFromEachSpeciesDuplicate();
    vector<CNeuralNet*> GetDuplicateBrainsForPopulation();
