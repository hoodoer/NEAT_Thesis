...
#define DUPLICATE_VECTOR_EXTRA_MEMORY 2
    ...
    m_dAvFitAdj(0)
{
    ...
    // Reserve vector space just in
    // case we're using multi-threaded duplicates
    // If you exceed the sizes, and the vector automagically
    // increases in size, the Genomes *will* be duplicated,
    // however the phenotype will be destroyed. And then
    // bad things happen. Robots with no brains, cats living with dogs,
    // and your software just won't work anymore.
    popGenomesDupVec.reserve(param_numExecutionThreads * DUPLICATE_VECTOR_EXTRA_MEMORY);
    popGenomesDupVec.resize(param_numExecutionThreads);

    bestGenomesDupVec.reserve(param_numExecutionThreads * DUPLICATE_VECTOR_EXTRA_MEMORY);
    bestGenomesDupVec.resize(param_numExecutionThreads);

    hallOfFameGenomesDupVec.reserve(param_numExecutionThreads * DUPLICATE_VECTOR_EXTRA_MEMORY);
    hallOfFameGenomesDupVec.resize(param_numExecutionThreads);

    speciesLeadersGenomesDupVec.reserve(param_numExecutionThreads * DUPLICATE_VECTOR_EXTRA_MEMORY);
    speciesLeadersGenomesDupVec.resize(param_numExecutionThreads);

    customArchiveGenomesDupVec.reserve(param_numExecutionThreads * DUPLICATE_VECTOR_EXTRA_MEMORY);
    customArchiveGenomesDupVec.resize(param_numExecutionThreads);

    for (int i = 0; i < param_numExecutionThreads; i++)
    {
        popGenomesDupVec[i].reserve(param_numAgents * DUPLICATE_VECTOR_EXTRA_MEMORY);
        bestGenomesDupVec[i].reserve(param_numBestAgentsPerTeam * DUPLICATE_VECTOR_EXTRA_MEMORY);
        hallOfFameGenomesDupVec[i].reserve(param_maxNumGenerations * DUPLICATE_VECTOR_EXTRA_MEMORY);
        speciesLeadersGenomesDupVec[i].reserve(param_targetNumSpecies * DUPLICATE_VECTOR_EXTRA_MEMORY);
        customArchiveGenomesDupVec[i].reserve(param_maxArchiveSize * DUPLICATE_VECTOR_EXTRA_MEMORY);
    }
    // End memory reservation junk for genome copies
...
//-------------------------------CreatePhenotypesDuplicate-----------------
//
//	cycles through all the members of the population and creates their
//  phenotypes. Returns a vector containing pointers to the new phenotypes.
//  This version of the function is for use by multi-threaded evaluation
//  implementations, each time it's called, a complete copy of the population
//  genomes is created, and a neural network is created from the copied genome.
//  this way, each thread using the neural network will have it's own copy.
//  These duplicates are cleared at the start of Epoch.
//-------------------------------------------------------------------------
vector<CNeuralNet*> Cga::CreatePhenotypesDuplicate()
{
    int initialVectorCapacity = popGenomesDupVec.capacity();

    vector<CNeuralNet*> brains;
    int depth;
    int duplicateVectorIndex = popGenomesDupVec.size();

    // Create a copy of the genomes (CGenome has a copy constructor)
    popGenomesDupVec.push_back(m_vecGenomes);


    for (unsigned int i = 0; i < popGenomesDupVec[duplicateVectorIndex].size(); i++)
    {
        depth = CalculateNetDepth(popGenomesDupVec[duplicateVectorIndex][i]);
        popGenomesDupVec[duplicateVectorIndex][i].SetDepth(depth);
        brains.push_back(popGenomesDupVec[duplicateVectorIndex][i].CreatePhenotype());
    }

    // Set the Clone ID number for these brains
    for (unsigned int i = 0; i < brains.size(); i++)
    {
        brains[i]->setCloneID(duplicateVectorIndex);
    }

    if (popGenomesDupVec.capacity() != initialVectorCapacity)
    {
        cout<<"Error! In Cga::CreatePhenotypeDuplicate, duplicate vector resized, previous copies are broken now"<<endl;
        cout<<"-- init capacity: "<<initialVectorCapacity<<", current capacity: "<<popGenomesDupVec.capacity()<<endl;
...
{
    // Clear out the duplicate vectors
    popGenomesDupVec.clear();
    bestGenomesDupVec.clear();
    hallOfFameGenomesDupVec.clear();
    speciesLeadersGenomesDupVec.clear();
    customArchiveGenomesDupVec.clear();
    ...
    case ORIGINAL_TOURNY_SELECT:
        ...
        if (NumSpawnedSoFar < param_numAgents)
        {
            ...
            if (!xorTestMode)
            {
                cout<<"--- We're short "<<Rqd<<" genomes, making up the difference"<<endl;
            }
...
//--------------------------- GetDuplicateBrainsForPopulation-------------
//
//  Like Epoch, it returns a vector of brains for the population, however
//  this function will copy the genomes prior to creating the brains and
//  returning them, to support multi-threaded processes that need their
//  own copies of the neural networks. Call Epoch between generations,
//  and call this function once for each copy you need of the neural
//  networks
//------------------------------------------------------------------------
vector <CNeuralNet*> Cga::GetDuplicateBrainsForPopulation()
{
    int initialVectorCapacity = popGenomesDupVec.capacity();
    ...
    if (popGenomesDupVec.capacity() != initialVectorCapacity)
    {
        cout<<"Error! In Cga::GetDuplicateBrainsForPopulation, duplicate vector resized, previous copies are broken now"<<endl;
        exit(1);
    }
...
//----------------- GetBestPhenotypesFromLastGenerationDuplicate---------
//
//  returns a std::vector of the n best phenotypes from the previous
//  generation, however it creates copies of the genomes before doing so
//  so that multiple processes will have their own copies to work with
//  Each time this is called, a new duplicate set of genomes is created.
//  These duplicates are cleared at start of Epoch.
//------------------------------------------------------------------------
vector <CNeuralNet*> Cga::GetBestPhenotypesFromLastGenerationDuplicate()
{
    int initialVectorCapacity = bestGenomesDupVec.capacity();

    vector<CNeuralNet*> brains;
    int depth;
    int duplicateVectorIndex = bestGenomesDupVec.size();


    // Create a copy of genomes (CGenome has a copy constructor)
    bestGenomesDupVec.push_back(m_vecBestGenomes);

    for (unsigned int i = 0; i < bestGenomesDupVec[duplicateVectorIndex].size(); i++)
    {
        depth = CalculateNetDepth(bestGenomesDupVec[duplicateVectorIndex][i]);
        bestGenomesDupVec[duplicateVectorIndex][i].SetDepth(depth);
        brains.push_back(bestGenomesDupVec[duplicateVectorIndex][i].CreatePhenotype());
    }
    ...
    if (bestGenomesDupVec.capacity() != initialVectorCapacity)
    {
        cout<<"Error! In Cga::GetBestPhenotypesFromLastGenerationDuplicate, duplicate vector resized, previous copies are broken now"<<endl;
        exit(1);
    }
...
//----------------- GetHallOfFamePhenotypesDuplicate --------------------
//
//  returns a std::vector of the phenotypes of the hall of fame,
//  however it creates copies of the genomes before doing so
//  so that multiple processes will have their own copies to work with.
//  Each time this is called, a new duplicate set of genomes is created.
//  These duplicates are cleared at the start of Epoch
//------------------------------------------------------------------------
vector<CNeuralNet*> Cga::GetHallOfFamePhenotypesDuplicate()
{
    int initialVectorCapacity = hallOfFameGenomesDupVec.capacity();

    vector<CNeuralNet*> brains;
    int depth;
    int duplicateVectorIndex = hallOfFameGenomesDupVec.size();

    // Create a copy of the genomes (CGenome has a copy constructor)
    hallOfFameGenomesDupVec.push_back(m_vecHallOfFameGenomes);

    for (unsigned int i = 0; i < hallOfFameGenomesDupVec[duplicateVectorIndex].size(); i++)
    {
        depth = CalculateNetDepth(hallOfFameGenomesDupVec[duplicateVectorIndex][i]);
        hallOfFameGenomesDupVec[duplicateVectorIndex][i].SetDepth(depth);
        brains.push_back(hallOfFameGenomesDupVec[duplicateVectorIndex][i].CreatePhenotype());
    }
    ...
    if (hallOfFameGenomesDupVec.capacity() != initialVectorCapacity)
    {
        cout<<"Error! In Cga::GetHallOfFamePhenotypesDuplicate, duplicate vector resized, previous copies are broken now"<<endl;
        exit(1);
    }
...
//----------------- GetCustomArchivePhenotypesDuplicate --------------------
//
//  returns a std::vector of the phenotypes in the custom archive,
//  however it creates copies of the genomes before doing so
//  so that multiple processes will have their own copies to work with.
...
//  These duplicates are cleared at the start of Epoch
//------------------------------------------------------------------------
vector<CNeuralNet*> Cga::GetCustomArchivePhenotypesDuplicate()
{
    int initialVectorCapacity = customArchiveGenomesDupVec.capacity();

    vector<CNeuralNet*> brains;
    int depth;
    int duplicateVectorIndex = customArchiveGenomesDupVec.size();

    // Create a copy of the genomes (CGenome has a copy constructor)
    customArchiveGenomesDupVec.push_back(m_vecCustomArchiveGenomes);

    for (unsigned int i = 0; i < customArchiveGenomesDupVec[duplicateVectorIndex].size(); i++)
    {
        depth = CalculateNetDepth(customArchiveGenomesDupVec[duplicateVectorIndex][i]);
        customArchiveGenomesDupVec[duplicateVectorIndex][i].SetDepth(depth);
        brains.push_back(customArchiveGenomesDupVec[duplicateVectorIndex][i].CreatePhenotype());
    }
    ...
    if (customArchiveGenomesDupVec.capacity() != initialVectorCapacity)
    {
        cout<<"Error! In Cga::customArchiveGenomesDupVec, duplicate vector resized, previous copies are broken now"<<endl;
        exit(1);
    }
...
//----------------- GetLeaderPhenotypesFromEachSpeciesDuplicate-------------
//
//  returns a std::vector of the phenotypes of the best of each species,
//  however it creates copies of the genomes before doing so, so that
//  multiple processes will have their own copies to work with.
//  Each time this is called, a new duplicate set of genomes is created.
//  These duplicates are cleared at the start of Epoch
//------------------------------------------------------------------------
vector<CNeuralNet*> Cga::GetLeaderPhenotypesFromEachSpeciesDuplicate()
{
    int initialVectorCapacity = speciesLeadersGenomesDupVec.capacity();

    m_vecSpeciesLeaderGenomes.clear();

    vector<CNeuralNet*> brains;
    int depth;
    int duplicateVectorIndex = speciesLeadersGenomesDupVec.size();
    ...
    // Creates a copy of the genomes (CGenome has a copy constructor)
    speciesLeadersGenomesDupVec.push_back(m_vecSpeciesLeaderGenomes);

    for (unsigned int i = 0; i < speciesLeadersGenomesDupVec[duplicateVectorIndex].size(); i++)
    {
        depth = CalculateNetDepth(speciesLeadersGenomesDupVec[duplicateVectorIndex][i]);
        speciesLeadersGenomesDupVec[duplicateVectorIndex][i].SetDepth(depth);
        brains.push_back(speciesLeadersGenomesDupVec[duplicateVectorIndex][i].CreatePhenotype());
    }
    ...
    if (speciesLeadersGenomesDupVec.capacity() != initialVectorCapacity)
    {
        cout<<"Error! In Cga::GetLeaderPhenotypesFromEachSpeciesDuplicate, duplicate vector resized, previous copies are broken now"<<endl;
        exit(1);
    }
...
void Cga::TestOldSpeciesForKilling()
{
    ...
    if (killableSpecies)
    {
        if (!xorTestMode)
        {
            cout<<"--->Killing species index "<<worstSpeciesIndex<<", it's the worst species past the age limit"<<endl;
        }
