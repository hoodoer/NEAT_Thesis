// Example of rotation and translation of a vector...
...
class CMatrix 
{
 public:
  ...
  inline CVec3 operator* (const CVec3 &Point ) const
    {
      double x = Point.x*mf[0] + Point.y*mf[4] + Point.z*mf[8]  + mf[12];
      double y = Point.x*mf[1] + Point.y*mf[5] + Point.z*mf[9]  + mf[13];
      double z = Point.x*mf[2] + Point.y*mf[6] + Point.z*mf[10] + mf[14]; 
      return CVec3( x, y, z );
    }

  // Rotate the *this matrix fDegrees counter-clockwise around a single axis( either x, y, or z )
  void Rotate( double fDegrees, int x, int y, int z )
    {
      CMatrix Temp;
  ...
  void RotateMatrix( double fDegrees, double x, double y, double z)
    {
      Identity();
      double cosA = cosf(fDegrees*TO_RAD);
      double sinA = sinf(fDegrees*TO_RAD);
      double m = 1.0f - cosA;
      mf[0] = cosA + x*x*m;
      mf[5] = cosA + y*y*m;
      mf[10]= cosA + z*z*m;
